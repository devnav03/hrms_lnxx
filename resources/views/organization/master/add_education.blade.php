            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header">
                        <h5 class="">Add Education</h5>
                    </div>
                    <div class="card-body">
                        <form class="forms-sample row" action="{{url('add-educations')}}" method="POST">
                            @csrf
                            <div class="col-sm-3">
                            ...
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="education_title" name="education_title" value="@if(!empty($update->education_title)){{$update->education_title}}@endif" placeholder="Enter Education Title" maxlength="50" required>
                    ...
                    <div class="card-header card-height">
                        <div class="row">
                            <div class="col-md-12 col-12">
                                <h5 class="" id="getCameraSerialNumbers">Education List</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
...
<script>
    $(document).ready(function () {
        var datatable = $('#example').dataTable({
            ajax: "{{url('ajax/get-education-master')}}",
            columns: [
                {data:'id',
                ...
                {data: null,
                    mRender:function ( data, type, row ) {
                        return '<a href="{{url("add-educations")}}/'+data.id+'" class="text-primary mx-2"><i class="fa fa-edit"></i></a>'+
                        '<a href="{{url("educations-delete")}}/'+data.id+'" class="text-danger delete-button"><i class="fa fa-trash"></i></a>';
                    }
                },
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    url: "{{url('ajax/get-default-education')}}",
                    type: "POST",
    ...
    $(document).on('click','.status_checks',function(){
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                    url: "{{url('ajax/get-status-education')}}",
                    type: "POST",
