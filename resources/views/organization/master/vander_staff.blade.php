            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header">
                        <h5 class="">Add Vander's Staff</h5>
                    </div>
                    <div class="card-body">
                        <form class="forms-sample row" action="{{url('vanders-staff')}}" method="POST">
                            @csrf
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="vanderstaff_id" value=" @if(!empty($update->id)){{$update->id}}@else 0 @endif ">
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Email *</label>
                                    <input type="email" class="form-control" id="email" name="email" value="@if(!empty($update->email)){{$update->email}}@endif" placeholder="Enter Email" maxlength="150" required>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Mobile *</label>
                                    <input type="number" class="form-control" id="mobile" name="mobile" value="@if(!empty($update->mobile)){{$update->mobile}}@endif" placeholder="Enter Mobile" maxlength="150" required>
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="vendor_id" name="vendor_id" required>
                                        @if(!empty($vendors))
                                            ...
                                            @foreach($vendors as $row)
                                                <option value="{{$row->id}}" data-id="{{$row->id}}" @if(!empty($update->vander_id)) @if($update->vander_id==$row->id) selected @endif @endif>{{$row->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                    ...
                    <div class="card-header card-height">
                        <div class="row">
                            <div class="col-md-12 col-12">
                                <h5 class="" id="getCameraSerialNumbers">Staff List</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
...
<script>
    $(document).ready(function () {
        ...
        var datatable = $('#example').dataTable({
            ajax: "{{url('ajax/get-vander-staff-list')}}",
            columns: [
                {data:'id',
                ...
                {data: null,
                    mRender:function ( data, type, row ) {
                        return '<a href="{{url("vanders-staff")}}/'+data.id+'" class="text-primary mx-2"><i class="fa fa-edit"></i></a>'+
                        '<a href="{{url("vander-delete")}}/'+data.id+'" class="text-danger delete-button"><i class="fa fa-trash"></i></a>';
                    }
                },
            ]
        });
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    url: "{{url('ajax/get-vander-staff-status')}}",
                    type: "POST",
