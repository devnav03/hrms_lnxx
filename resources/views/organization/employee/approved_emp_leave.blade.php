                ...
                <div class="col-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="">Search Employee Approve Leave</h5>
                        </div>
                        <div class="card-body">
          ...
          e.preventDefault();
            ...
            var datatable = $('#example').dataTable({
                "ajax": function (data, callback, settings) {
                    $.ajax({
                    url: "{{url('ajax/get-emp-approved-leave-data')}}",
                    dataType:"json",
                    type: 'POST',
                    data: $('form').serialize(),
                ...
                columns: [
                    ...
                    {data: null,
                        mRender:function ( data, type, row ) {
                            return '<a class="lable-success">'+data.status+'</a>';
                        }
                    },
                    {data: null,
                        mRender:function ( data, type, row ) {
            ...
            $.ajax({
                ...
                success:function(xhr){
                    if(xhr.status==200){
                        ...
                        '<td><a class="lable-success">'+xhr.data.status+'</a>'+
                        '</td></tr>';
                        $('#leave_data').html(html);
                        $('#leave_id').html('<span class="btn btn-info btn-sm" data-dismiss="modal">skip</span>');
                        spinner.hide();
                    }
                }
            });
