            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header">
                        <h5 class="">Add Email Template</h5>
                    </div>
                    <div class="card-body">
                        <form class="forms-sample row" action="{{url('add-email-template')}}" method="POST">
                            @csrf
                    ...
                    <div class="card-header card-height">
                        <div class="row">
                            <div class="col-md-12 col-12">
                                <h5 class="" id="getCameraSerialNumbers">Email Template List</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    url: "{{url('ajax/get-status-email-template')}}",
                    type: "POST",
...
<script>
    $(document).ready(function () {
        var datatable = $('#example').dataTable({
            ajax: "{{url('ajax/get-email-template-list')}}",
            columns: [
                {data:'id',
                ...
                {data: null,
                    mRender:function ( data, type, row ) {
                        return '<a href="{{url("add-email-template")}}/'+data.id+'" class="text-primary mx-2"><i class="fa fa-edit"></i></a>'+
                        '<a href="{{url("email-template-delete")}}/'+data.id+'" class="text-danger delete-button"><i class="fa fa-trash"></i></a>';
                    }
                },
            ]
        });
