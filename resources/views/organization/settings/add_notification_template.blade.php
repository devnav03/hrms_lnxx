            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header">
                        <h5 class="">Add Notification Template</h5>
                    </div>
                    <div class="card-body">
                        <form class="forms-sample row" action="{{url('add-notification-template')}}" method="POST">
                            @csrf
                    ...
                    <div class="card-header card-height">
                        <div class="row">
                            <div class="col-md-12 col-12">
                                <h5 class="" id="">Notification Template List</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    url: "{{url('ajax/get-status-notification-template')}}",
                    type: "POST",
...
<script>
    $(document).ready(function () {
        var datatable = $('#example').dataTable({
            ajax: "{{url('ajax/get-notification-template-list')}}",
            columns: [
                {data:'id',
                ...
                {data: null,
                    mRender:function ( data, type, row ) {
                        return '<a href="{{url("add-notification-template")}}/'+data.id+'" class="text-primary mx-2"><i class="fa fa-edit"></i></a>'+
                        '<a href="{{url("notification-template-delete")}}/'+data.id+'" class="text-danger delete-button"><i class="fa fa-trash"></i></a>';
                    }
                },
            ]
        });
