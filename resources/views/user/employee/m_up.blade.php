                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                @if($row->data_type=='file')
                                    <span class="{{$row->form_column}}_id"><input type="hidden" name="{{$row->form_column}}" value="<?php echo !empty($updatedata[$row->form_column][$i]) ? $updatedata[$row->form_column][$i]:'';?>"></span>
                                @endif
                                <input type="{{$row->data_type}}" @if($row->form_column=='profile') onchange="loadFile(event)" @endif class="form-control file_{{$row->form_column}}_id" name="{{$row->form_column}}[]" placeholder="Enter {{$row->form_name}}" value="<?php echo !empty($updatedata[$row->form_column][$i]) ? $updatedata[$row->form_column][$i]:'';?>"
                                @if($row->is_required==1) required @endif 
                                @if(!empty($row->data_length)) maxlength="{{$row->data_length}}" @endif
                                <?php if(!empty($row->pattern)){ echo $row->pattern;}?> @if($row->editable==0) readonly @endif>
                                <div class="help-block alert-{{$row->form_column}}" style="font-size: 12px;"></div>
                                @if($row->data_type=='file') @if(!empty($updatedata[$row->form_column][$i])) <a href="{{asset($updatedata[$row->form_column][$i])}}" class="download-link download-{{$i}}" download="">Download</a> @endif @endif
                                <script>
                                    $(document).ready(function() {
                                        $('.file_{{$row->form_column}}_id').change(function() {
