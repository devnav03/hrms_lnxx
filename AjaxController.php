                    ...
                    <tr>
                       <?php if(!empty($result)){ 
                        ...
                        ?>
                            ...
                            <tr>
                                <td>CandidateName</td>
                                <td><?php echo $results_data->candidate_name;?> </td>
                            </tr>
                            <tr>    
                            ...
                            <tr> 
                                ...
                                <td><?php echo $results_data->candidate_email;?> </td>
                            </tr>
                            <tr> 
                            ...
                            <tr> 
                                ...
                                <td><?php echo $results_data->candidate_salary;?> </td>
                            </tr>
                            <tr> 
                            ...
                            <td> <?php if(!empty($results_data->candidate_resume)) {?><a target="_blank" class="btn btn-primary btn-xs" href="<?php echo $results_data->candidate_resume;?>" download><i class="fa fa-download">  &nbsp; &nbsp; Download Resume</i></a><?php } ?>
                            </td>
                            </tr>
                            </table>
                            <?php } } ?> 
                    ...
                    <tr>
                       <?php if(!empty($getRequiredDoc)){ 
                        ?>
                           ...
                           <?php  foreach($getRequiredDoc as $requiredDoc){?>
                            <tr>
                                ...
                                <td><?php echo $requiredDoc->document_file;?> </td>
                        ...
                        <table class="table table-condensed">
                            ...
                            <td><?php echo $signedDoc->document_file;?> </td>
                            <td><i class="fa fa-eye"></i></td>
                            </tr>
                            <?php } ?>
                            </table>
    ...
    if(!empty($data)){
        $updated_rec = SendOfferLettersToCandidate::where(['organisation_id'=>$user_id,'candidate_id'=>$candidate_id,'tracking_status'=>'1'])->first();
         if(!empty($updated_rec)){
                $this->SendHrOfferLetterMail($updated_rec, $updated_rec->id);
        }
...
public function UploadSignedDoc(Request $request){  
    ...
    if($files=$request->file('upload_document')){ 
        ...
        $files->move('public/uploads/candidate-upload-signed-doc',$newuploadedfilename);
        $document_title= $request->filename;
        $document_file= $newuploadedfilename;     

        $user_id = Auth::user()->id;
        $uploadSignedDoc = new CandidateSignedDocument();
...
public function UploadDocumentForVisaApproval(Request $request){
    ...
    $result = SendHrRequest::where(['organisation_id'=>$user_id,'id'=>$request->candidate_rec_id])->first();
    $date = date('Y-m-d H:i:s');

    if(!empty($result)){  

    $get_duplicate = SendVisaApproval::where(['organisation_id'=>$user_id,'id'=>$candidate_id,'status'=>'0'])->first(); 
     if(empty($get_duplicate)){  
        $managers=User::where(['organisation_id'=>$user_id,'id'=>$manager_name_id])->first(); 
      ...
      if(!empty($data)){
        ...
        if(!empty($getdata)) {
            $this->SendeVisaApprovalMail($getdata);
        }else{
...
public function SendeVisaApprovalMail($data){
...
/*=============END GET DAILY ATTENDENCE  ====================*/
    ...
    /*-----------------START SEND OFFER LETTER TO CANDIDATE THROUTH MAIL-----------------*/
       public function SendHrOfferLetterMail($data, $id){
        $user_id = Auth::user()->id;
        $email = array($data->email);
        ...
        try {
            ...
            Mail::send(['html'=>'email.offer_letter'], $template_data, function ($message) use ($email, $template_data, $document_file, $document_title) {
            $message->to($email)
            ->from("lnxx@gmail.com")
